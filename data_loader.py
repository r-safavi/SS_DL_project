...
class PongDataLoader2D():
    ...
    def get_data(self,num_sequences=16,sequence_length=15):
        ...
        for k,episode in enumerate(randomly_chosen_episodes):
            randomly_chosen_step = np.random.choice(self.step_list[sequence_length+1:-1],1)
            for i in np.arange(sequence_length,0,-1):
                ...
                action_file_name = 'ep%05dstep%05d_action'%(episode,randomly_chosen_step-i+1)
...
class PongDataLoader3D():
    def __init__(self,zip_name = 'dataset40.zip'):
        ...
        while True:
            ...
            if file_name[-5:] == 'state':
                my_array = self.numpy_files[file_name]
                self.first_dim,self.second_dim,self.depth = my_array.shape
                break
            else: 
                i+=1
    ...
    def get_data(self,num_sequences=16,sequence_length=15):
        ...
        Will return an array of size (num_sequences,sequence_length+1,self.first_dim,self.second_dim)
        '''
        state_sequences = np.zeros([num_sequences,sequence_length+1,self.first_dim,self.second_dim,self.depth])
        action_sequences = np.zeros([num_sequences,sequence_length+1])
        randomly_chosen_episodes = np.random.choice(self.episode_list,size=num_sequences,replace=True)
        for k,episode in enumerate(randomly_chosen_episodes):
            randomly_chosen_step = np.random.choice(self.step_list[sequence_length+1:-1],1)
        ...
        action_sequences[(action_sequences==2)|(action_sequences==4)] = 1
        action_sequences[(action_sequences==3)|(action_sequences==5)] = 2
        return state_sequences.transpose(0,1,4,2,3)/255,action_sequences